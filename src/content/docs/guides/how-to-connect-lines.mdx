---
title: Connect intersecting lines on a SadConsole surface
description: Use the ConnectLines method to join overlapping box and line glyphs with proper junction characters on a SadConsole surface.
---
...
![Four boxes and a single line. The glyphs for each box don't join each other cleanly.](/article-assets/images/guides/how-to-connect-lines/before.png)

The [ConnectLines(ISurface, int\[\])](/reference/sadconsole.cellsurfaceeditor#connectlinesisurface-int) editing method connects those characters to make intersecting lines. The method takes the array that's composed of line character glyphs in a specific sequence and connects them together. There are at least two line sequences provided by SadConsole, [ICellSurface.ConnectedLineThin](/reference/sadconsole.icellsurface#connectedlinethin) and [ICellSurface.ConnectedLineThick](/reference/sadconsole.icellsurface#connectedlinethick). Lines from one set won't intersect and auto connect to another set.
...
![Four boxes and a single line. The glyphs for each box cleanly join each other.](/article-assets/images/guides/how-to-connect-lines/after.png)
