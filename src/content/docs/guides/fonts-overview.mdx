...
<Aside type="tip">
Community user **tf01** has created a [font generator tool](https://github.com/tfi01/SadFontsUtil) that can help you create font image and config files.
</Aside>
...
To use a custom font on startup, adjust the game config that starts SadConsole. For more information, see [Use your own font as the default font](/guides/config-overview/#use-your-own-font-as-the-default-font).
...
To change the default font after SadConsole starts, set the `SadConsole.Game.Instance.DefaultFont` property to a different font instance. Changing the default font **only** affects object created after the change.
...
Fonts can be preloaded with the game config. For more information, see [Use a delegate to configure the font options](/guides/config-overview/#use-a-delegate-to-configure-the-font-options).
